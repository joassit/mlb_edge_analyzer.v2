...
2026-07-09 -- usando exclusivamente las funciones OFICIALES de
...
# Revisión diaria de AYER (2026-07-08) -- ya tiene resultado real.
try:
    out["daily_review_yesterday"] = compute_daily_review("2026-07-08")
except Exception as e:
    out["daily_review_yesterday_error"] = str(e)

# Revisión diaria de HOY, por si algún juego temprano ya liquidó.
try:
...
try:
    # Distribución por fecha: todo lo que hay en game_analysis, agrupado.
    ...
    for d, rows in sorted(by_date.items()):
        # dedup por game_pk quedandose con el id mas alto (mismo criterio que compute_metrics)
        seen = set()
        deduped = []
    ...
    # Fichas de HOY (Parte IX): todas las filas de game_analysis de TODAY,
    # con su(s) pick(s) asociados, para reconstruir pitchers/parque/clima/
    # proyecciones/prob/edge/ev/fair-odds/discrepancia direccional.
    ...
    # Validez de probabilidades (chequeo de sanidad, igual que Parte VI del informe anterior)
...
# Metadatos de git (mismo criterio que el informe anterior: commit corto + hitos)
try:
