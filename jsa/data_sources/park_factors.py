...
Tabla estatica (misma fuente/valores que `mlb_edge_analyzer.v2/data/
park_factors.py`) -- no cambia partido a partido, no vale la pena
consultarla por API. Debe actualizarse 1-2 veces por temporada desde una
...
def get_park_info(home_team_id: int) -> dict:
    """Park factor + coordenadas del estadio local. Cae a valores neutrales
    si el equipo no esta en la tabla."""
    return BALLPARKS.get(
        home_team_id,
