...
Postgres real de producción -- secrets.DATABASE_URL) que arma un JSON con
TODO lo necesario para el Technical Performance Report de 2026-07-21 --
usando exclusivamente las funciones OFICIALES de tracking/results_tracker.py
(nunca recalculando a mano). Primer informe que incluye las 4 funciones
analíticas nuevas (edge lift, ROI por probabilidad, descomposición,
métricas rodantes) agregadas en main commit bb8ee65.
"""
import base64
import gzip
import json
...
out["edge_lift_30"] = compute_edge_lift(days=30)
out["pick_performance_by_probability_30"] = compute_pick_performance_by_probability(days=30)

rolling = compute_rolling_metrics(window=30)
out["rolling_metrics"] = rolling
...
try:
    ...
    # Descomposición de un par de juegos representativos de hoy (el de
    # mayor edge real y uno forzado) -- no de TODOS, para no inflar el
    # payload; el resto queda disponible vía decompose_game_projection()
    # si hace falta auditar otro puntual.
    real_fichas = [f for f in fichas if f["pick"] and not f["pick"]["forced"]]
    if real_fichas:
        top_edge_ficha = max(real_fichas, key=lambda f: f["pick"]["edge"] or 0)
        out["decomposition_top_edge_game"] = {
            "game_pk": top_edge_ficha["game_pk"],
            "matchup": f"{top_edge_ficha['away_team']} @ {top_edge_ficha['home_team']}",
            "decomposition": decompose_game_projection(top_edge_ficha["game_pk"], TODAY),
        }
